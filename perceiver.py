...
def attend(
    ...
    if attention_mask is not None:
        ...
        attention = jnp.where(attention_mask[:, None, None, :, :], attention, -large_k)
    ...
    if attention_mask is not None:
        ...
        summed = jnp.where(wipe_attn[:, None, :, :], jnp.zeros_like(summed), summed)
    return summed
...
class MultiChannelLinear(hk.Module):
    ...
    def __call__(self, inputs):
        """
        Args:
            inputs: [batch, num_pathways, latents_per_pathway, latent_dim]
        """
...
def layer_norm(x: jnp.array, name: Optional[str] = None):
    return hk.LayerNorm(axis=-1, create_scale=True, create_offset=True, name=name)(x)


def make_cross_attention_mask(query_mask: jnp.array, kv_mask: jnp.array):
...
class Attention(hk.Module):
    ...
    def __init__(
        self,
        num_q_pathways: int = 8,
        num_kv_pathways: int = 8,
        num_heads_per_pathway: int = 2,
        init_scale: float = 1.0,
        ...
        self._num_q_pathways = num_q_pathways
        self._num_kv_pathways = num_kv_pathways
        self._num_heads_per_pathway = num_heads_per_pathway
        self._init_scale = init_scale
...
class SelfAttention(hk.Module):
    ...
    def __call__(self, inputs, *, attention_mask=None, is_training):
        ...
        attention = Attention(
            ...
            num_kv_pathways=self._num_pathways,
            num_heads_per_pathway=self._num_heads_per_pathway,
...
class CrossAttention(hk.Module):
    ...
    def __call__(self, inputs_q, inputs_kv, *, attention_mask=None, is_training):
        ...
        attention = Attention(
            num_q_pathways=self._num_pathways,
            num_kv_pathways=1,  # Only allow cross attention with single pathway input.
            num_heads_per_pathway=self._num_heads_per_pathway,
            init_scale=self._att_init_scale,
            dropout_prob=dropout_attn_prob,
...
class Perceiver(hk.Module):
    ...
    def __init__(
        ...
        encoder: Module,
        decoder: Module,
        input_preprocessor: Optional[Module] = None,
        output_postprocessor: Optional[Module] = None,
        name="perceiver",
    ):
        super().__init__(name=name)
...
class PerceiverEncoder(hk.Module):
    ...
    def __init__(
        ...
        num_blocks: int = 8,
        num_pathways_per_block: int = 8,
        num_self_attends_per_block: int = 6,
        z_index_dim: int = 512,
        num_z_channels: int = 1024,
        num_channels_for_pathways: int = 256,
        concat_or_add_pos: str = "concat",
        qk_channels: Optional[int] = None,
        v_channels: Optional[int] = None,
        num_cross_attend_heads_per_pathway: int = 1,
        num_self_attend_heads_per_pathway: int = 8,
        ...
        use_query_residual: bool = True,
        name: str = "perceiver_encoder",
    ):
        super().__init__(name=name)

        # Check that we can use multihead-attention with these shapes.
        num_self_attend_heads_per_block = (
            num_pathways_per_block * num_self_attend_heads_per_pathway
        )
        if num_z_channels % num_self_attend_heads_per_block != 0:
            raise ValueError(
                f"num_z_channels ({num_z_channels}) must be divisible by"
                f" num_self_attend_heads ({num_self_attend_heads_per_block})."
            )
        num_cross_attend_heads_per_block = (
            num_pathways_per_block * num_cross_attend_heads_per_pathway
        )
        if num_z_channels % num_cross_attend_heads_per_block != 0:
            raise ValueError(
                f"num_z_channels ({num_z_channels}) must be divisible by"
                f" num_cross_attend_heads ({num_cross_attend_heads_per_pathway})."
            )

        # Check that we can use multiple pathways with these shapes.
        if z_index_dim % num_pathways_per_block != 0:
            raise ValueError(
                f"z_index_dim ({z_index_dim}) must be divisible by"
                f" num_pathways_per_block ({num_pathways_per_block})."
            )

        self._input_is_1d = True
        self._num_blocks = num_blocks
        self._num_pathways_per_block = num_pathways_per_block
        ...
        self.cross_attend = CrossAttention(
            dropout_prob=dropout_prob,
            num_pathways=num_pathways_per_block,
            num_heads_per_pathway=num_cross_attend_heads_per_pathway,
    ...
    def latents(self, inputs: jnp.array):
    ...
    def __call__(
        ...
        *,
        is_training: bool,
        input_mask: Optional[jnp.array] = None,
    ):
        attention_mask = None
...
class BasicDecoder(AbstractPerceiverDecoder):
    ...
    def __init__(
        ...
        output_w_init: Optional[Initializer] = None,
        concat_preprocessed_input: bool = False,
        num_heads: int = 1,
        name: str = "basic_decoder",
    ...
    def __call__(self, query, z, *, is_training, query_mask=None):
        ...
        decoding_cross_attn = CrossAttention(
            dropout_prob=0.0,
            num_pathways=1,
            num_heads_per_pathway=self._num_heads,
            widening_factor=1,
            shape_for_attn="kv",
            qk_channels=self._qk_channels,
            v_channels=self._v_channels,
...
class ClassificationDecoder(AbstractPerceiverDecoder):
    ...
    def __init__(
        self, num_classes: int, name: str = "classification_decoder", **decoder_kwargs
    ):
        super().__init__(name=name)
